...
<table width='100%' border='0' cellpadding='0' cellspacing='0' style='padding: 0; margin: 0;'>
    <{foreach item=shout from=$shouts}>
        <tr style="width: 100%;" class='<{cycle values="even,odd"}>'
            title='<{$shout.time}><{if $xoops_isadmin}> [<{$shout.ip}>]<{/if}>'>
            ...
            <td class='sideboxcontent'>
                <{if $shout.uid neq 0}>
                    <span style="font-weight: bold;"><a href='<{$xoops_url}>/userinfo.php?uid=<{$shout.uid}>' target='_top'><{$shout.uname}></a>:</span>
                <{else}>
                    <span style="font-weight: bold;"><{$shout.uname}><span style="color: #990000; ">*</span>:</span>
                <{/if}>
                <{$shout.message}>
            </td>
        </tr>
